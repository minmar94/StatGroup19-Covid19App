...
mirrRich <- function(pars, ti, tk) 
{
  ...
  l1p10h2p2Inf <- log(1+10^(h2*p2))
  l1p10h2p2 <- ifelse(is.infinite(l1p10h2p2Inf), h2*p2*log(10),
                      l1p10h2p2Inf)
  l1p10h1p1Inf <- log(1+10^(h1*p1))
  l1p10h1p1 <- ifelse(is.infinite(l1p10h1p1Inf), h1*p1*log(10),
                      l1p10h1p1Inf)
  logr2 <- logr1+s2*l1p10h2p2-s1*l1p10h1p1
...
d1MRich8 <- function(pars, ti, tk)
{
  ...
  ll2aux <- ifelse(is.infinite(ll2auxInf), ll2hp, ll2auxInf)
  l1p10hp <- 1+10^(h1*p1)
  l2p10hp <- 1+10^(h2*p2)
  ll1p10hpInf <- log(l1p10hp)
  ll2p10hpInf <- log(l2p10hp)
  ll1p10hp <- ifelse(is.infinite(ll1p10hpInf), ll1hp, ll1p10hpInf)
  ll2p10hp <- ifelse(is.infinite(ll2p10hpInf), ll2hp, ll2p10hpInf)
...
d2MRich8 <- function(pars, ti, tk)
{
  ...
  # Lambda 2, h2
  dl2h2h2 <- d1l2h2 + h2^2*log(10)^2*exp(logr1-s1*ll1p10hp+s2*ll2p10hp+
                                           h2*p2*log(10)+log(s2)+
                                           -s2*ll2aux)*
    (
      exp(log(p2^2)+log(s2)+h2*p2*log(10)-2*ll2p10hp)+
        exp(log(p2^2)-ll2p10hp)+
        -p2*pmt2*exp(log(s2)-ll2aux+ll2hp-ll2p10hp)+
        -p2*pmt2*exp(log(s2)-ll2aux+ll2hp-ll2p10hp)+
        -p2*pmt2*exp(-h2*tilde2*log(10)-ll2aux)+
        exp(log(pmt2^2)+log(s2)-2*ll2aux+h2*(p2-2*tilde2)*log(10))+
        -exp(log(p2^2)-2*ll2p10hp+h2*p2*log(10))+
  ...
  dl2h2p1 <- -log(10)^2*h1*h2*exp(logr1+log(s2)+s2*ll2p10hp+h2*p2*log(10)-s2*ll2aux+
                                    log(s1)+(-s1-1)*ll1p10hp+h1*p1*log(10)) *
    (p2*l2p10hp^(-1)-pmt2*exp(-h2*tilde2*log(10)-ll2aux))
  dl2h2p2 <- log(10)^2*h2^2*exp(logr1+log(s2)-s1*ll1p10hp+h2*p2*log(10)+
                                  s2*ll2p10hp-s2*ll2aux)*
    (
      + p2 * exp(log(s2)-2*ll2p10hp+h2*p2*log(10)) +
        - pmt2*exp(log(s2)-ll2p10hp+ll2hp-ll2aux) +
        + p2 * (1+10^(h2*p2))^(-1) +
        - pmt2*exp(-h2*tilde2*log(10)-ll2aux) +
        - p2 * exp(log(s2)-ll2p10hp+ll2hp-ll2aux) +
        + pmt2*exp(log(s2)-2*ll2aux+h2*(p2-2*tilde2)*log(10)) +
        + (h2*log(10)*(1+10^(h2*p2)))^(-1) +
        - p2*exp(h2*p2*log(10)-2*ll2p10hp) +
  ...
  dl2s2s2 <- d1l2s2 + (ll2p10hp-ll2aux)^2*
    exp(logr1-s1*ll1p10hp+s2*ll2p10hp-s2*ll2aux+
          2*log(s2))
                      ...
                      maxiter=1e4, runs=500, nBoot=1000)
{
  ...
  # Double-check correct monotonicity
  if (all(diff(count)>=0))
  {
    monotone <- T
  }
  ...
  rg3 <- ga("real-valued", function(x)  lik(x),
            lower=apply(rbind(rg@solution, rg2@solution), 2, min)-10,
            upper=apply(rbind(rg@solution, rg2@solution), 2, max)+5,
