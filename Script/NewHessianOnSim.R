...
set.seed(12345)
truePars <- c(log(0.1), log(1000), 0.1, log(40), log(3))
nts <- 100
ts <- 1:nts
...
dd <- density(c(rep(ts, countSim), nts+(nts-rep(ts, countSim))), from = 1, to = nts)
countSmooth <- dd$y*2*sum(countSim)
...
# Optim
inits2 <- optim(inits, function(x) -likPois(x, cc=countSim, ti=ts), 
                method="BFGS")$par

# Genetic algorithm
maxiter=1e4
runs=500
rg <- ga("real-valued", function(x) likPois(x, cc=countSim, ti=ts), 
         lower=rep(-20,length(inits)), upper=rep(20,length(inits)),
         maxiter=maxiter/2, run=runs, optim=TRUE, 
         suggestions=rbind(inits, inits2))

rg2 <- ga("real-valued", function(x) likPois(x, cc=countSim, ti=ts), 
          lower=rep(-20,length(inits)), upper=rep(20,length(inits)), 
          maxiter=maxiter/2, run=runs, optim=TRUE)
...
curve(Richards(x, pars=inits2), lwd=2, col=3, xlim=c(0, nts), add=T)
curve(Richards(x, pars=truePars), lwd=2, col=1, lty=2, xlim=c(0, nts), add=T)
