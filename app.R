...
#source("Script/FunzioniUtili2.R")
                 ...
                 tabPanel(title = "Panoramica",
                          ...
                          div(style="margin-top:-3.5em",
                              ...
                              fluidRow(tags$hr(),
                                       ...
                                       column(width = 6, 
                                              htmlOutput(outputId = "Defins"), style="text-align:justify;padding:20px;")
                              )
                          ), 
                              ...
                              sidebarLayout(
                                sidebarPanel(width = 3,
                                             radioButtons(
                                               ...
                                               choices = list("Cumulati positivi", "Diagnosticati positivi"),
                            ...
                            sidebarLayout(
                              sidebarPanel(
                                ...
                                pickerInput(inputId = "VarForModel", 
                                            label = "Seleziona una variabile",
                                            choices = list("Cumulati positivi", "Diagnosticati positivi", "Attualmente ricoverati", 
                                                           "Terapia intensiva",
                                ...
                                tags$br(),
                                uiOutput(outputId = "HowtoRedMirr"),
                                uiOutput(outputId = "AddReducedMirrored"),
                                sliderInput(
...
server <- function(input, output, session){
  ...
  # Cos'è la COVID19
  output$COV19 <- renderText({"La COVID-19 (dall'inglese COronaVIrus Disease 2019) o malattia respiratoria acuta da SARS-CoV-2 (dall'inglese Severe acute respiratory syndrome coronavirus 2),
  ...
  <ul><li><b>Cumulati positivi =</b>  diagnosticati positivi + deceduti + dismessi guariti </li>
  <li><b>Diagnosticati positivi =</b> ricoverati con sintomi + terapia intensiva + isolamento domiciliare</li>
  ...
  output$DonughtPlot <- renderPlotly({
    ...
    donut_out <- do_ciambella(da = dati_Ita, reg = NULL, variable = input$VarRow1)
    
    if(input$DonBarPerReg){
      donut_out <- do_ciambella(da = dati_reg, reg = input$DisplayRegRow1, variable = input$VarRow1)
    }
  ...
  output$BarStackPos <- renderPlotly({
    ...
    bar_out <- do_barplot_ts(da = dati_Ita, reg = NULL, variable = input$VarRow1)
    
    if(input$DonBarPerReg){
      bar_out <- do_barplot_ts(da = dati_reg, reg = input$DisplayRegRow1, variable = input$VarRow1)
    }
  ...
  output$TSIncrPerc <- renderPlotly({
    ...
    ts_plot <- do_ts(da = dati_Ita, reg = NULL, 
                     varsel = as.character(variable()), datasel = as.character(datetoselect()), 
                     is.incrementi = input$IncrementiSiNo, tipo.incremento = input$IncrPercSiNo)
    if(input$TSRegioneSiNo){
      ts_plot <- do_ts(da = dati_reg, reg = as.character(Regselected()), 
                       varsel = as.character(variable()), datasel = as.character(datetoselect()), 
                       is.incrementi = input$IncrementiSiNo, tipo.incremento = input$IncrPercSiNo)
    }
  ...
  # Add Mirrored Reduced explanation
  output$HowtoRedMirr <- renderUI({
    if(!outputModello()$monot){
      HTML(paste(h5("Utilizzare sempre (se si vuole), tranne se la variabile selezionata è 'Deceduti' o 'Cumulati positivi'")))
    }else{
      return(NULL)
    }
  })
  # Add Mirrored Reduced
  output$AddReducedMirrored <- renderUI({
    if(!outputModello()$monot){
      checkboxInput(
        inputId = "AddReducedMirrored",
        label = "Confronta con Mirrored Richards Ridotta", 
        value = F
      )
    }else{
      return(NULL)
  ...
  output$SummaryModel <- reactive({
    ...
    if(!is.character(outputModello())){
      if(input$ModRegioneSiNo){
        return(
          summary_out_model(outputmod = outputModello(), varest = input$VarForModel, resdata = residents, 
                            reg = input$SelRegModel, is.reg = T, monotone = outputModello()$monot)
        )
      }else{
        summary_out_model(outputmod = outputModello(), varest = input$VarForModel, resdata = residents, is.reg = F, reg = NULL, 
                          monotone = outputModello()$monot)
      }
