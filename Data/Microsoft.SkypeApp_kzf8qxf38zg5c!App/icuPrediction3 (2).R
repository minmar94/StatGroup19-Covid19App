rm(list=ls())
load("Data/Microsoft.SkypeApp_kzf8qxf38zg5c!App/DatiRegione13Apr_recode.RData")
...
da$capienza=rep(da.pred$capienza,each=max(da$ti))
    ...
    fit.loss=glmer(icu~ti+I(ti^2/100)+offset(log(residents))+((1+ti)|region)+((0+I(ti^2/100))|region),data=da[da$ti<max(da$ti),],family=poisson)
                    ...
                    ba.loss=t(simplify2array(by(da[da$ti<max(da$ti),],da[da$ti<max(da$ti),]$region,fup)))
...
library(xtable)
print(xtable(pr.oggi,digits=0,caption="Previsioni Ricoveri in Terapia Intensiva per il 14 Aprile"),type="html",file="ICU.predictions.html",include.rownames=FALSE)
